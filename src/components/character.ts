...
import { Vector3, PhysicsImpostor, Mesh, SpriteManager, Sprite, KeyboardEventTypes, PointerEventTypes, LinesMesh, Angle, Color3, Space } from 'babylonjs';
...
export default class Character extends GameObject {
  ...
  constructor(level: Level, mesh?: Mesh) {
    ...
    this.turtleState = TurtleState.Swimming;
    ...
    const spriteManagerFlare = new SpriteManager("turtleSprite", "./public/img/Sprite-Tortue-Active-4x4-min.png", 1, 256, this.Scene);
    this.turtle = new Sprite("turtle", spriteManagerFlare);
    this.turtle.playAnimation(0, 14, true, 120);
    this.turtle.size = 4;
    ...
    this.Scene.onPointerObservable.add((mouseData) => {
      if (mouseData.type == PointerEventTypes.POINTERMOVE) {
        ...
        this.MainMesh.rotate(Vector3.Right(), mouseData.event.movementY / 100);
        this.MainMesh.rotate(Vector3.Up(), mouseData.event.movementX / 100, Space.WORLD);
      }
    });
    ...
    this.Env.registerFunctionBeforeUpdate(() => {
      ...
      //slerp ?
      const limitValue = Angle.FromRadians(Math.PI / 6).degrees();
